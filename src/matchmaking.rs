    ...
    loop {
        match host.service(1000).expect("ENet service failed") {
            ...
            Some(Event::Receive {
                ...
                let connect_code_encoded: Vec<u8> = parsed_json["search"]["connectCode"].members().map(|value| {
                    return value.as_u8().unwrap();
                }).collect();
                let (connect_code, _enc, errors) = SHIFT_JIS.decode(&connect_code_encoded);

                match message_type {
                    Ok(MatchmakingMessageType::CreateTicket) => {
                        println!("create-ticket for {:?} {}", play_mode, connect_code);
                        sender.send_packet(
                            Packet::new(
